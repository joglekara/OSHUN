...
//  *= 
SHarmonic1D& SHarmonic1D::multiply_guard(const double & d, const size_t Nbc){
...
SHarmonic1D& SHarmonic1D::Dp()
{
    ...
    ///
        ...
        // double seventeensixth(static_cast<complex<double> >(17./6.));
...
//             for (size_t p(0); p<nump();++p)
//             {
//                 if ((std::isnan((*sh)(p,ix,iy))) || (std::isnan((*sh)(p,ix,iy).imag())))
//                 {
...
// //--------------------------------------------------------------
// //  Copy assignment operator
//     Field2D& Field2D::operator=(const double& d){
//         *fi = d;
//         return *this;
//     }
...
//  Copy assignment operator
EMF1D& EMF1D::operator=(const double& d){
    for (size_t i=0; i < dim() ; ++i)
        (*fie)[i] = d;
    return *this;
}
...
// //  Copy assignment operator
//     EMF2D& EMF2D::operator=(const double& d){
//         for(int i=0; i < dim() ; ++i)  
//             (*fie)[i] = d;
//         return *this;
//     }
...
//  *=
DistFunc1D& DistFunc1D::multiply_guard(const double & d, const size_t Nbc){
    #pragma omp parallel for num_threads(Input::List().ompthreads)
    for(size_t i = 0; i < dim() ; ++i) {
...
void DistFunc1D::Filterp(){
    ...
    for(size_t i = 0; i < dim() ; ++i) 
    {
        (*df)[i] *= double(exp(-36.*pow(i/(dim()-1),36)));
        // (*df)[i].Filterp(i);
...
valarray<double> DistFunc1D::getdensity(){
    ...
    valarray<double > vr(Algorithms::MakeCAxis(
    static_cast<double > (0.0),static_cast<double >(1.0),(*df)[0].nump()));
    // valarray<double > dvr(vr);xwx

    vr[0] = static_cast<double > (0.5*dp[0]);
    // dvr[0] = static_cast<double >(dp[0]);
    for (size_t ip(1); ip < dp.size(); ++ip)
    {
        vr[ip]  = static_cast<double > (dp[ip-1]);        
        vr[ip] += static_cast<double > (dp[ip]);
        vr[ip] *= static_cast<double > (0.5);
        vr[ip] += vr[ip-1];
    }
    // dvr[0] = static_cast<double >(dp[0]);

    for (size_t i(0); i<(*df)[0].numx();++i){
        out[i] = (4.0*M_PI*Algorithms::moment((*df)[0].xVec(i),vr,2.0));
        // out[i] = (4.0*M_PI*Algorithms::moment((*df)[0].xVec(i),vr,dvr,2.0));
    }
...
valarray<double> DistFunc1D::getdensity() const {
    ...
    valarray<double > vr(Algorithms::MakeCAxis(
            static_cast<double > (0.0),static_cast<double >(1.0),(*df)[0].nump()));
    valarray<double > dvr(vr);

    vr[0] = static_cast<double > (0.5*dp[0]);
    dvr[0] = static_cast<double >(dp[0]);
    for (size_t ip(1); ip < dp.size(); ++ip)
    {
        dvr[ip] = static_cast<double >(dp[ip]);
        vr[ip]  = static_cast<double > (dp[ip-1]);        
        vr[ip] += static_cast<double > (dp[ip]);
        vr[ip] *= static_cast<double > (0.5);
        vr[ip] += vr[ip-1];
...
valarray<double> DistFunc1D::getcurrent(size_t dir){
    ...
    if (dir == 0)
    {
        #pragma omp parallel for num_threads(Input::List().ompthreads)
        for (size_t i = 0; i<(*df)[0].numx();++i){
            out[i] = (4.0/3.0*M_PI*charge/ma)*(Algorithms::moment((*df)[1].xVec(i),vr,3.0));
        }
    }
    else if (dir == 1)
    {
        #pragma omp parallel for num_threads(Input::List().ompthreads)
        for (size_t i = 0; i<(*df)[0].numx();++i){
            out[i] = (8.0/3.0*M_PI*charge/ma)*(Algorithms::moment((*df)[2].xVec(i),vr,3.0));
        }
    }
    else if (dir == 2)
    {
...
Array2D<double> DistFunc1D::getcurrent() const{
    ...
    for (size_t i = 0; i<(*df)[0].numx();++i)
    {
        out(0,i) = (current_c1)*(Algorithms::moment((*df)[1].xVec(i),vr,3.0));

        out(1,i) = (current_c2)*(Algorithms::moment((*df)[2].xVec(i),vr,3.0));
...
valarray<double> DistFunc1D::getrelativisticcurrent(size_t dir){
    ...
    {
        for (size_t i(0); i<(*df)[0].numx();++i){
            out[i] = (4.0/3.0*M_PI*charge/ma)*(Algorithms::relativistic_invg_moment((*df)[1].xVec(i),vr,3.0));
        }
    }
    else if (dir == 1)
    {
        for (size_t i(0); i<(*df)[0].numx();++i){
            out[i] = (8.0/3.0*M_PI*charge/ma)*(Algorithms::relativistic_invg_moment((*df)[2].xVec(i),vr,3.0));
        }
    }
    else if (dir == 2)
    {
...
Array2D<double> DistFunc1D::getrelativisticcurrent() const{
    ...
    for (size_t i(0); i<(*df)[0].numx();++i)
    {
        out(0,i) = (current_c1)*(Algorithms::relativistic_invg_moment((*df)[1].xVec(i),vr,3.0));

        out(1,i) = (current_c2)*(Algorithms::relativistic_invg_moment((*df)[2].xVec(i),vr,3.0));
...
valarray<double> DistFunc1D::getpressure(){
    ...
    for (size_t i(0); i<(*df)[0].numx();++i){
        out[i] = (4.0/3.0/ma*M_PI*Algorithms::moment((*df)[0].xVec(i),vr,4.0));
    }
...
//                     out(ix,iy) = (4.0*M_PI*Algorithms::moment((*df)[0].xVec(ix,iy),vr,2.0));
//                 }
//             }
//         return out;
//     }    
...
//             for (size_t iy(0); iy<(*df)[0].numy();++iy){
//                 for (size_t ix(0); ix<(*df)[0].numx();++ix){
//                     out(ix,iy) = (current_c1*Algorithms::moment((*df)[1].xVec(ix,iy),vr,3.0));
//                 }
//             }
//         }   
//         else if (dir == 1)
...
//             for (size_t iy(0); iy<(*df)[0].numy();++iy){
//                 for (size_t ix(0); ix<(*df)[0].numx();++ix){
//                     out(ix,iy) = (current_c2*Algorithms::moment((*df)[2].xVec(ix,iy),vr,3.0));
//                 }
//             }
//         }
//         else if (dir == 2)
...
//             for (size_t iy(0); iy<(*df)[0].numy();++iy){
//                 for (size_t ix(0); ix<(*df)[0].numx();++ix){
//                     out(0,ix,iy) = (current_c1)*(Algorithms::moment((*df)[1].xVec(ix,iy),vr,3.0));
            
//                     out(1,ix,iy) = (current_c2)*(Algorithms::moment((*df)[2].xVec(ix,iy),vr,3.0));
...
//             for (size_t iy(0); iy<(*df)[0].numy();++iy)
//             {
//                 out(ix,iy) = (current_c1)*(Algorithms::relativistic_invg_moment((*df)[1].xVec(ix,iy),vr,3.0));
//             }
//         }
//     }
//     else if (dir == 1)
...
//             for (size_t iy(0); iy<(*df)[0].numy();++iy)
//             {
//                 out(ix,iy) = (current_c2)*(Algorithms::relativistic_invg_moment((*df)[2].xVec(ix,iy),vr,3.0));
//             }
//         }
//     }
//     else if (dir == 2)
...
//         for (size_t iy(0); iy<(*df)[0].numy();++iy)
//         {
//             out(0,ix,iy) = (current_c1)*(Algorithms::relativistic_invg_moment((*df)[1].xVec(ix,iy),vr,3.0));

//             out(1,ix,iy) = (current_c2)*(Algorithms::relativistic_invg_moment((*df)[2].xVec(ix,iy),vr,3.0));
...
//             for (size_t iy(0); iy<(*df)[0].numy();++iy){
//                 for (size_t ix(0); ix<(*df)[0].numx();++ix){
//                     out(ix,iy) = (4.0/3.0/ma*M_PI*Algorithms::moment((*df)[0].xVec(ix,iy),vr,4.0));
//                 }
//             }
...
//                 for (size_t ix(0); ix<(*df)[0].numx();++ix){
//                     for (size_t p(0); p<(*df)[indx].nump();++p){
//                         if (  std::isnan((*df)[indx](p,ix,iy)) || std::isnan((*df)[indx](p,ix,iy).imag())   )
//                         {   
...
//  *=
State1D& State1D::multiply_guard(const double & d, const size_t Nbc){
    for(size_t s(0); s < ns; ++s){
        // (*sp)[s] *= d;
...
//     }
// //  =
//     State2D& State2D::operator=(const double& d){
