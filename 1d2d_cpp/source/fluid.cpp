...
void Hydro_Functor::operator()(const State1D& Yin, State1D& Yslope, double time, double dt){}
//-------------------------------------------------------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------
   : XX1(Nl+1,Nm+1), XX2(Nl+1,Nm+1), XX3(Nl+1,Nm+1), XX4(Nl+1,Nm+1),
     ...
     Hp0(Nl+1), H(dp.size(),Nx), G(dp.size(),Nx), TMP(dp.size(),Nx),
     pr(Algorithms::MakeCAxis(static_cast<complex<double> >(0.),
                             static_cast<complex<double> >(1.0),
                             dp.size())),
    // pr(Algorithms::MakeCAxis_cmplx((0.),dp)),
    invdp(pr),
     invpr(pr), szx(Nx)
     {
//      - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        {
                pr[ip] = pr[ip-1] + 0.5*(dp[ip-1]+dp[ip]);
                // std::cout << "\n Epr[" << ip << "] = " << pr[ip] << std::endl;
        }
        invdp[0] = 0.;
        for (size_t i(1); i < pr.size()-1; ++i)
        {
            invdp[i] = -1.0/(0.5*(dp[i-1]+dp[i+1])+dp[i]);
        }
...
//       Inverted momentum axis
         ...
         idp = 1.0; //(-1.0)/ (2.0*(pmax-pmin)/double(Np)); // -1/(2dp)
